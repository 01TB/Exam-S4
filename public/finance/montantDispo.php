...
<body>
    ...
    <script>
        ...
        function chargerInterets() {
            ...
            ajax("GET", `/solde?${params.toString()}`, null, (data) => {
                updateTable(data);
            }, (error) => {
                document.querySelector('.error').style.display = 'block';
        ...
        function updateTable(data) {
            ...
            if (data.length === 0) {
                ...
                tr.innerHTML = `<td colspan="5" class="font-['Inter'] text-center">Aucun résultat trouvé</td>`;
                ...
                data.forEach(montant => {
                    ...
                    tr.innerHTML = `
                        ...
                        <td class="font-['Inter']">${montant.total_depots}</td>
                        <td class="font-['Inter']">${montant.total_prets}</td>
                        <td class="font-['Inter']">${formatMontant(montant.tresorerie_disponible)} €</td>
                    `;
                    tbody.appendChild(tr);
                });
            }
